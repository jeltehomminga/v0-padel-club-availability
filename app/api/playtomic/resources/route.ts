...
export async function GET(request: NextRequest) {
  ...
  for (const url of urls) {
    try {
      ...
      console.error(`[v0] Error with URL ${url}:`, error)
  ...
  console.error(`[v0] All URLs failed for tenant ${tenantId}`)
  return NextResponse.json({ error: "Failed to fetch resources from all endpoints" }, { status: 500 })
}
